...
const mutations = {
  SET_FOOD_POSTS(state, posts) {
    state.foodPosts = posts
  },
  ADD_FOOD_POST(state, post) {
    state.foodPosts.unshift(post)
  },
  SET_TOPICS(state, topics) {
    state.topics = topics
  },
  UPDATE_FOOD_LIKE(state, { id, isLiked }) {
    const post = state.foodPosts.find(p => p.id === id)
    if (post) {
      post.isLiked = isLiked
      post.likes += isLiked ? 1 : -1
    }
  },
  UPDATE_FOOD_BOOKMARK(state, { id, isBookmarked }) {
    const post = state.foodPosts.find(p => p.id === id)
    if (post) {
